...
Vue.component('castle-banners', {
    template: `<div class="banners">
        ...
        <banner-bar class="food-bar" clolor="#288339" :ratio="foodRatio" />
        <!-- Health -->
        <img class="health-icon" src="svg/health-icon.svg" />
        <bubble type="health" :value="player.health" :ratio="healthRatio" />
        <banner-bar class="health-bar" clolor="#9b2e2e" :ratio="healthRatio" />
        </div>`,
    props: ['player'],
    computed: {
        foodRatio () {
...
Vue.component ('banner-bar', {
    ...
    watch: {
        targetHeight (newValue, oldValue) {
            ...
            .onUpdate( function () {
                vm.height = this._object.value.toFixed(0)
            })
            .start()
        },
    },
...
Vue.component('cloud', {
    ...
    methods: {
        ...
        startAnimation (delay = 0) {
            ...
            .delay(delay)
            .onUpdate(function () {
                vm.setPosition(this._object.value, top)
            })
            .onComplete(() => {
            // With a random delay
